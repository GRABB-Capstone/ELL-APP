...
open class SCLAlertView: UIViewController {
    ...
    func keyboardWillShow(_ notification: Notification) {
        ...
        if let userInfo = notification.userInfo {
            if let beginKeyBoardFrame = (userInfo[UIKeyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue.origin.y {
                if let endKeyBoardFrame = (userInfo[UIKeyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.origin.y {
    ...
    open func showTitle(_ title: String, subTitle: String, duration: TimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil) -> SCLAlertViewResponder {
        ...
        if showCloseButton {
            ...
            let _ = addButton(txt, target:self, selector:#selector(SCLAlertView.hideView))
        }
